...
def create_sample_dataset() -> list[Observation]:
    ...
    regression a much denser and smoother 2D surface to learn from so the
    fitted curve/response surface looks more natural.
    The collected table is kept intact, and additional midpoint observations are
    generated by averaging neighbouring measured values. This gives the symbolic
    regression a denser and more natural 2D surface to learn from so temperature
    and glucose both appear in the discovered equation more consistently.
    """
    collected_rows = [
        (20.0, 0.20, 0.83),
        (20.0, 0.15, 0.74),
...
def protected_divide(numerator: float, denominator: float, epsilon: float = 1e-6) -> float:
    ...
    for temperature in temperature_values:
        for glucose_left, glucose_right in zip(glucose_values, glucose_values[1:]):
            midpoint_glucose = (glucose_left + glucose_right) / 2.0
            midpoint_voltage = (mean_lookup[(temperature, glucose_left)] + mean_lookup[(temperature, glucose_right)]) / 2.0
            augmented_dataset.append(Observation(midpoint_glucose, temperature, midpoint_voltage))

    for glucose in glucose_values:
        for temperature_low, temperature_high in zip(temperature_values, temperature_values[1:]):
            midpoint_temperature = (temperature_low + temperature_high) / 2.0
            midpoint_voltage = (mean_lookup[(temperature_low, glucose)] + mean_lookup[(temperature_high, glucose)]) / 2.0
            augmented_dataset.append(Observation(glucose, midpoint_temperature, midpoint_voltage))
    ...
    for temperature_low, temperature_high in zip(temperature_values, temperature_values[1:]):
        for glucose_left, glucose_right in zip(glucose_values, glucose_values[1:]):
            ...
            centre_glucose = (glucose_left + glucose_right) / 2.0
            cell_average = (
                mean_lookup[(temperature_low, glucose_left)]
                + mean_lookup[(temperature_low, glucose_right)]
                + mean_lookup[(temperature_high, glucose_left)]
                + mean_lookup[(temperature_high, glucose_right)]
            ) / 4.0
            augmented_dataset.append(Observation(centre_glucose, centre_temperature, cell_average))
